function mstSerpMain() {
   ...
   var results = document.getElementsByClassName('algo'); /* main */


    // Calculate results per page
    var resultStats = document.querySelector('.title.mb-0.fz-14');
   // var resultStats = '<span style="color:inherit;" class=" fz-14 lh-22">About 68,700,000 search results</span>';
    ...
    for (var countActual = 0; countActual < results.length; countActual++, countDisplay++) {
        ...
        // Skip if URL is invisible (PAA box, other search features)
           console.log(results[countActual]);
        var height = window.getComputedStyle(results[countActual].querySelector('.compTitle  h3')).height;
        ...
        results[countActual].parentNode.append(counter);
    }
