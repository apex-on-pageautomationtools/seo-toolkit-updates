...
def request_indexing_via_session(session_id, property_url, email, urls,
    ...
    Deliberately never routes through a proxy - this drives an already
    logged-in, local Google account session, and suddenly changing its exit
    IP via proxy risks tripping Google's own "unusual activity" challenges
    rather than helping anything (confirmed team call: no proxy makes sense
    here, unlike the unauthenticated Wayback submission path)."""
    if log_fn is None:
        log_fn = print
